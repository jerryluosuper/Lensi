...
LastEditTime: 2022-03-17 12:01:35
...
def web_360_search(app_name,limmit_num):
    ...
    for i in range(0,limmit_num_real):
        ...
        #网址处理
        baoku_search_all = [baoku_name.strip(",").encode('ascii').decode('unicode_escape'),baoku_version.strip(","),baoku_detail_text.strip(",").encode('ascii').decode('unicode_escape'),baoku_id,baoku_download_url.strip(","),baoku_icon_url,"360"]
        #整理格式
...
def web_qq_search(app_name,limmit_num):
    ...
    for i in range(0,limmit_num_real):
        ...
        #网址处理
        qq_search_all = [qq_name.encode('ascii').decode('unicode_escape'),qq_version,qq_detail.encode('ascii').decode('unicode_escape'),qq_id,qq_download_url,"qq"]
        #整理格式
...
def winget_search(app_name,limmit_num):#winget 搜索解析
    ...
    search_result_list = f.readlines()[3:]#去除前三行干扰 有bug
    if search_result_txt.find("找不到与输入条件匹配的程序包。") != -1 or len(search_result_list)==0:
        return search_result_list_all #异常处理——找不到或根本不存在？
    if search_result_list[len(search_result_list)-1] == "<由于结果限制而截断了其他条目>" or search_result_list[len(search_result_list)-1] == "\n":
